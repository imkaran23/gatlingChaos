...
class GetCachedMessages extends Simulation {


  val httpProtocol = http
    .baseUrl("http://localhost:8080/api/v1/cache") // 5
    .inferHtmlResources()
    .acceptHeader("*/*")
  ...
  val getCachedMessages = scenario("Get Cached Messages")
    .repeat(1) {
      exec(http("Get Cached Messages")
        .get("http://localhost:8080/api/v1/cache") // 5
 ...
 setUp(
    getCachedMessages.inject(constantConcurrentUsers(RunConfigurations.NO_OF_USERS)
